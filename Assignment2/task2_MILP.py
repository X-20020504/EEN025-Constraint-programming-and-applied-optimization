...
for j in range(n_jobs):
    ...
    model.addConstr(t[j, 0] >= moving_time, name=f"job's_operating_sequence")
...
for j in range(n_jobs):
    for h in range(n_machines-1):
        ...
        distance = A_star(start, end)
        moving_time = ceil(distance / 5)
        model.addConstr(t[j, h+1] >= t[j, h] + q[j][h] + moving_time, name=f"job's_operating_sequence")
...
for j in range(n_jobs):
    ...
    distance = A_star(start, end)
    moving_time = ceil(distance/5)
    model.addConstr(C >= t[j, n_machines-1] + q[j][n_machines-1] + moving_time, name=f"job_{j}_finishing_time")
